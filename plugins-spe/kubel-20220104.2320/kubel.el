...
;; Version: 1.0
;; Package-Version: 20220104.2320
;; Package-Commit: 68d2925c7942039e3fb3eb6c113adec5369c6c72
...
(defun kubel--exec-to-string (cmd &optional noerror)
  "Replace \"shell-command-to-string\" to log to process buffer.

CMD is the command string to run.

Expects command to use `kubectl' bin.
Throws an error if not found unless NOERROR is non-nil."
  (if (executable-find "kubectl" 'honor-dd)
      (progn
        (kubel--log-command "kubectl-command" cmd)
        (shell-command-to-string cmd))
    (unless (not (null noerror))
      (error "executable `kubectl' not found"))))
