...
(defun kubel--get-context-arg ()
  "Utility function to return the proper context argument."
  (unless (equal kubel-context "")
    (list "--context" kubel-context)))

(defun kubel--get-namespace-arg ()
  "Utility function to return the proper namespace argument."
...
(defun kubel--get-context-namespace ()
  ...
  (append
   (kubel--get-context-arg)
   (kubel--get-namespace-arg)))
         ...
         (progn
           (unless kubel--can-get-namespace-cached
             (setq kubel--can-get-namespace-cached
                   (string-match-p "yes\n"
                                   (kubel--exec-to-string
                                    (format "%s %s auth can-i list namespaces" kubel-kubectl (s-join " " (kubel--get-context-arg))))))))
         kubel--can-get-namespace-cached)))

(defun kubel--get-namespace (&optional force)
  "Get namespaces for current context, try to recover from cache first."
  (unless (and (not force) kubel--namespace-list-cached)
    (setq kubel--namespace-list-cached
          (split-string (kubel--exec-to-string
                         (format "%s %s get namespace -o jsonpath='{.items[*].metadata.name}'" kubel-kubectl (s-join " " (kubel--get-context-arg)))) " ")))
...
(defun kubel--fetch-context ()
  "Fetch current context from kubectl."
  (let ((fetch-raw-res (kubel--exec-to-string (concat kubel-kubectl " config current-context") )))
...
(defun kubel--init-context ()
  "Set initial `kubel-context' from kubectl."
  (setq kubel-context (kubel--fetch-context)))
...
(defun kubel--fetch-api-resource-list ()
  ...
  (split-string (kubel--exec-to-string
                 (format "%s %s api-resources -o name --no-headers=true" kubel-kubectl (s-join " " (kubel--get-context-arg)))) "\n" t))
...
DIRECTORY is optional for TRAMP support."
  ...
  (message (concat "buff name5: " (buffer-name (current-buffer))))
  (message (concat "kubel-context 5: " kubel-context))
...
DIRECTORY is optional for TRAMP support."
  ...
  (let ((default-directory (or directory default-directory)))
    (when (not (executable-find kubel-kubectl 'honor-dd))
      (user-error "No kubectl program (%s) at current location" kubel-kubectl)))

  (let* ((context (kubel--fetch-context))
         (kubel-context context)
         (name (kubel--buffer-name))
