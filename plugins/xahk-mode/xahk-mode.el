...
;; Copyright Â© 2008, 2009, 2010, 2011, 2012 by Xah Lee
...
;; Open the file, then type â€œAlt+x eval-bufferâ€. You are done. Open
;; any ahk script, then type â€œAlt+x xahk-modeâ€, you'll see the
;; source code syntax colored.

;; To have emacs automatically load the file when it restarts, and
;; automatically use the mode when opening files ending in â€œ.ahkâ€, do this:

;; â‘  Put the file ã€”xahk-mode.elã€• in the dir ã€”~/.emacs.d/ã€•
;; â‘¡ Put the following lines in your emacs init file (usually at ã€”~/.emacsã€•).
...
;; version 1.2.2, 2012-05-21 modified syntax table so â€œ_â€ is part of word.
;; version 1.2.1, 2011-10-15 Minor changes. No visible behavior change.
;; version 1.2, 2010-02-17 fixed a defect where if source contains â€œ"C:\"â€, everything after is badly syntax colored. Thanks to â€œxinlu.hâ€ and â€œiain.tuddenhamâ€. Detail at http://code.google.com/p/ergoemacs/issues/detail?id=66
  ...
  (with-output-to-temp-buffer "*About xahk-mode*"
    (princ
     (concat "Mode name: xahk-mode.\n\n"
             ...
             "To see inline documentation, type â€œAlt+x `describe-mode'â€ while you are in xahk-mode.\n\n"
             "Home page: URL `http://xahlee.info/mswin/emacs_autohotkey_mode.html' \n\n")
     )
    )
  )

;; implementation using â€œnewcomment.elâ€.
...
Keywords include all AHK's event handlers, functions, and CONSTANTS."
  ...
  (let ((posEnd (point))
    ...
    (cond ((eq maxMatchResult t))
          ((null maxMatchResult)
           (message "Can't find completion for â€œ%sâ€" meat)
           (ding))
