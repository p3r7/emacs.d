...
(defalias 'mwe--flet 'flet)
(when (require 'noflet nil 'noerror)
  (defalias 'mwe--flet 'noflet))
...
SLIST  list of sub-token where current sexp is built from
  ...
  (mwe--flet ((mwe:make-overlay (beg end)
...
(defun mwe:forward-sexp (&optional arg)
  "Clone of fun `forward-sexp' wrapping scan-sexps w/
`ignore-errors'."
  (interactive "^p")
  (or arg (setq arg 1))
  (goto-char (or (ignore-errors (scan-sexps (point) arg)) (buffer-end arg)))
  (if (< arg 0) (backward-prefix-chars)))

(defun mwe:thing-at-point--end-of-sexp ()
...
"
  ...
  (cond ((looking-at "(")
	 (let* ((point (prog1 (point) (forward-char)))
		...
		(epoint (progn (mwe:thing-at-point--end-of-sexp) (point))))
